...
class PrsoxrLoader:
    ...
    **kwargs:
        ...
        with open(file, "r") as f:
            for i, line in enumerate(f):
                if "DATA" in line:
                    header_line = i
        return pd.read_csv(
            file, skiprows=header_line, header=1, delimiter="\t"
        )  ###################
    ...
    def cleanup_metadata(self):
        ...
        self.data["beam_current"] = self.data["beam_current"].apply(
            lambda x: x if x > 50 else 1
        )
        self.data["i0"] = self.data["i0"].apply(lambda x: x if x > 0 else 1)
        self.data["exposure"] = self.data["exposure"].apply(
        ...
        if (
            ...
            sam_z_move = (
                self.data["sam_z"].diff().abs() > 0.0
            )  # First move of sam_z to return to blocking the beam
            sam_z_move.loc[0] = False  # change NaN to False
            sam_z_move_index = self.data["sam_z"][sam_z_move].index + 1
            begin_refl_angle = self.data["sam_th"][sam_z_move_index].iloc[
                0
            ]  # First angle to collect data
            begin_ccd_angle = self.data["det_th"][sam_z_move_index].iloc[
                0
            ]  # First detector angle to collect data
            sam_th_offset = np.round(
                (begin_ccd_angle / 2 - begin_refl_angle), 4
        ...
        self.data["q"] = self.data.apply(
            lambda df: units.theta_to_q(np.deg2rad(df["sam_th"]), df["wavelength"]),
            axis=1,
        )

    def clean_images(self):
        # Cleanup some potential issues if parameters did not load
        dx = self.process_vars[
            "trim_x"
        ]  # How much to trim from the edges of the image (x dimension)
        dy = self.process_vars[
            "trim_y"
        ]  # How much to trim from the edges of the image (y dimension)
        ...
        self.data["filtered_image"] = self.data.progress_apply(
            lambda df: median_filter(
                df["raw_image"][dx:-dx, dy:-dy], size=self.process_vars["filter_size"]
            ),
            axis=1,
        )
        # Dezinger the images
        self.data["zinged_image"] = self.data.apply(
            lambda df: image.dezinger_image(
                df["raw_image"][dx:-dx, dy:-dy], med_result=df["filtered_image"]
            ),
            axis=1,
        )
    ...
    def generate_series_mask(self):
        ...
        for xy in mask_loc:
            distance_mask = (
                np.sqrt((grid_map[0] - xy[0]) ** 2 + (grid_map[1] - xy[1]) ** 2) < r
            )  # Go point-by-point and find the local 10 pixel circle
            mask = mask | distance_mask  # Add those points to be integrated.

        self.mask = (
            mask  # Mask does not move outside of this initial area. No odd jumps.
        )
    ...
    def process_images(self):
        ...
        self.data["reduced_image"] = self.data.apply(
            lambda df: df["zinged_image"], axis=1
        )
        ...
        self.data["dark"] = self.data.apply(
            lambda df: df["reduced_image"][df["roi_d"]], axis=1
        )
        # Sum up all the pixels within the roi's that amount to a 'point detector'
        self.data["counts_spot"] = self.data.apply(lambda df: df["spot"].sum(), axis=1)
        ...
        self.data["is_saturated"] = self.data.apply(
            lambda df: self.check_saturation(
                df["reduced_image"], threshold=self.process_vars["saturate_threshold"]
            ),
            axis=1,
        )
    ...
    def update_roi(self, beam_spot):  # Calculate the beam dimensions
        ...
        slx = slice(x_low_bound, x_low_bound + w)
        sly = slice(y_low_bound, y_low_bound + h)
    ...
    def locate_beam_byscan(self, save_mask=True):
        ...
        for i in tqdm(range(len(self)), desc="Finding Beam centers:"):
            ...
            if scan > current_scan or i == 0:
                current_scan = scan
                beam_spot, mask = self.find_maximum(
                    self.data["filtered_image"].iloc[i], init=None, w=w, h=h
                )
            else:
                prev_beamspot = self.data["beam_spot"].iloc[i - 1]
                beam_spot, mask = self.find_maximum(
                    self.data["filtered_image"].iloc[i], init=prev_beamspot, w=w, h=h
                )
            if save_mask:
    ...
    def find_maximum(self, init_image, init=None, w=20, h=20):  # Calculate the spot
        ...
        if isinstance(init, tuple):
            ...
            distance_mask = (
                np.sqrt((grid_map[0] - init[0]) ** 2 + (grid_map[1] - init[1]) ** 2) < r
            )
            masked_image = np.where(distance_mask, image, 0)
            yloc, xloc = np.unravel_index(
                np.argmax(masked_image), masked_image.shape
            )  # Get the peak position of the direct beam, easy
    ...
    def check_spot(self, fits_index, d=1):
        ...
        print(
            "Highest Pixel Counts: {} [counts]".format(df["zinged_image"].iloc[0].max())
        )
        ...
        if d:
            ...
            ax[0, 0].imshow(
                df["zinged_image"].iloc[0], norm=mpl_colors.LogNorm(), cmap=default_cmap
            )
            ax[0, 0].set_title("Raw Image")
            ax[0, 1].imshow(
                df["filtered_image"].iloc[0],
            ...
            ax[0, 1].plot(
                df["beam_spot"].iloc[0][1], df["beam_spot"].iloc[0][0], "ro", ms=2
            )
            ...
            ax[2, 0].imshow(
                df["spot"].iloc[0], norm=mpl_colors.LogNorm(), cmap=default_cmap
            )
            ax[2, 0].set_title("Beam Shape log-scale")

            ax[2, 1].imshow(
                df["dark"].iloc[0], norm=mpl_colors.LogNorm(), cmap=default_cmap
            )
    ...
    def display_fits(self, fits_index, bs=True):
        ...
        ax[0, 0].set_title("Raw Image")
        ax[0, 1].imshow(
            df["filtered_image"].iloc[0], norm=mpl_colors.LogNorm(), cmap=default_cmap
        )
        ax[0, 1].set_title("Filtered Image")
    ...
    def normalize_scan(self, df):
        ...
        try:
            i0_cutoff = (
                df.loc[np.abs(df["sam_z"].diff()) > 0.1].index[0] + 1
            )  # The first position where we move Z into the beam
        ...
        df["R_err"] = np.sqrt(
            (df["counts_refl"] / i0) ** 2
            * ((df["counts_err"] / df["counts_refl"]) ** 2 + (i0_err / i0) ** 2)
        )
    ...
    def find_stitch_points(self, df):
        ...
        for motor in stitch_motors:
            if (
                "sam_th" in motor
            ):  # This is the only motor that it matters which direction it moves
    ...
    def calc_scale_factors(self, df):
        ...
        for i in range(len(df)):  # row in df.iterrows():
            ...
            for j, val in enumerate(
                df["sam_th"].iloc[prev_mark_index:i]
            ):  # Start cycling from the beginning
            ...
            stitch_pre = (
                df[["sam_th", "R"]]
                .iloc[ipre]
                .loc[df["counts_ratio"] > self.process_vars["stitch_cutoff"]]
            )
            stitch_post = (
                df[["sam_th", "R"]]
                .iloc[ipost]
            ...
            safe_stitch_values = list(
                set(stitch_pre["sam_th"]) & set(stitch_post["sam_th"])
            )
